<?php
$title = 'Statistics Active OpenSim Grids';
...
{
    $normalized = normalize_grid_uri($value);
    return $normalized;
}
...
function build_grid_info_urls($loginUri)
{
    ...
    if (!empty($parts['host'])) {
        ...
        $hostOnlyHttp = 'http://' . $hostPort . '/get_grid_info';
        $hostOnlyHttps = 'https://' . $hostPort . '/get_grid_info';
        $urls[] = $hostOnlyHttp;
        $urls[] = $hostOnlyHttps;

        // Fallback when login URI includes path like /cgi-bin/login.cgi.
...
<head>
    ...
    <title>Statistics Active OpenSim Grids</title>
    <style>
        :root {
            --bg: #f4f6f7;
            --panel: #ffffff;
...
<div class="wrap">
    <div class="card">
        <h1>Statistics Active OpenSim Grids</h1>
        <p class="meta">Quelle: include/gridlist.csv + Robust DB (GridUser, userinfo) + oeffentliche Grid-Daten via HTTP GET /get_grid_info. Vorbild: hypergridbusiness.com/statistics/active-grids/</p>

        <div class="toolbar">
            <a class="btn" href="?download=1">CSV Download</a>
            <span class="meta">Datensaetze: <?php echo (int) count($rows); ?></span>
            <span class="meta">Exportdatei: include/groundgridlist.csv</span>
            <span class="meta">Grid-Info Refresh: <?php echo (int) $refreshCount; ?>/<?php echo (int) $gridInfoRefreshPerRequest; ?></span>
        </div>
